...
const { hasBrandSignal } = require("./trendFilter");
...
function calculateProductScore(keyword, product, index = 0) {
  ...
  let score = w.base;
  if (full && title.includes(full)) score += w.fullKeyword;
  ...
  if (hasBrandSignal(keyword) && hasBrandSignal(product.title)) score += w.brandModel;
