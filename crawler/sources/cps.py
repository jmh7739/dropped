...
_KW_SLUG = [
    ...
    (("노트북", "마우스", "키보드", "모니터", "ssd", "그래픽카드", "rtx",
      "cpu", "조립pc", "게이밍pc", "공유기", "웹캠", "ram", "메모리"), "digital"),
    # 'usb' 단독은 USB전원 램프·선풍기 등 비디지털도 오매칭 → 제거(메모리/ssd로 잡음).
    (("갤럭시", "아이폰", "버즈", "폰케이스", "휴대폰케이스", "핸드폰케이스",
      "보조배터리", "태블릿", "아이패드", "충전기"), "mobile"),
    (("냉장고", "세탁기", "청소기", "에어프라이어", "가습기", "선풍기", "드라이어",
      "면도기", "에어컨", "정수기", "밥솥", "인덕션", "건조기", "tv", "티비", "모니터암"), "appliance"),
    # 패션/의류/신발 — 신발류는 전부 패션으로. (브랜드는 _BRAND_SLUG에서 2차로.)
    (("셔츠", "팬츠", "니트", "코트", "원피스", "운동화", "신발", "슬리퍼", "백팩",
      ...
      "홈웨어", "머플러", "목도리", "스카프", "선글라스", "캐리어", "손목시계"), "fashion"),
    # '립' 단독은 이클립스·클립·튤립을 오매칭 → 립스틱/립밤 등 구체어로.
    (("스킨", "로션", "크림", "세럼", "에센스", "마스크팩", "선크림", "쿠션팩트",
      "립스틱", "립밤", "립글로스", "립틴트", "향수", "샴푸", "클렌징",
      "파운데이션", "틴트", "미스트", "앰플", "토너"), "beauty"),
    ...
    (("멸치", "오징어", "쌀", "김치", "라면", "과자", "견과", "커피", "육포", "어묵",
      ...
      "꿀 ", "벌꿀", "꿀단지", "즙", "티백", "생수", "간식", "찜", "탕", "국", "건어물", "황태", "쥐포", "곱창",
      ...
      "족발", "보쌈", "다시마", "미역", "조미김",
      "아이스크림", "빙수", "젤라또", "셔벗", "월병", "약과", "양갱",
      # 과일 — '배 '는 배송/배터리 오매칭 피하려 뒤 공백 포함
      "배 ", "사과", "포도", "감귤", "귤", "딸기", "수박", "참외", "멜론", "복숭아",
      "샤인머스켓", "한라봉", "블루베리", "과일",
      # 음료
      "환타", "사이다", "스프라이트", "탄산", "주스"), "food"),
    (("사료", "고양이", "강아지", "반려", "애견", "캐츠", "세제", "휴지", "청소",
      "수납", "주방", "냄비", "프라이팬", "세탁", "건조대", "정리", "밀폐", "텀블러", "그릇"), "living"),
]


# 브랜드 → slug (2차). 상품 자체 키워드가 안 잡혔을 때만 브랜드로 보조 분류.
#   카테고리가 모호하지 않은(거의 패션/아웃도어/신발) 브랜드만 넣는다.
#   삼성/LG/애플 등 여러 카테고리에 걸치는 브랜드는 넣지 않음(상품 키워드가 판단).
_BRAND_SLUG = {
    "fashion": (
        "나이키", "아디다스", "뉴발란스", "아식스", "퓨마", "컨버스", "크록스",
        "호카", "데상트", "휠라", "리복", "험멜", "스케쳐스", "블랙야크",
        "노스페이스", "코오롱스포츠", "컬럼비아", "디스커버리", "아이더", "네파",
        "라푸마", "몽벨", "프로스펙스", "유니클로", "스파오",
    ),
}
...
def _slug_by_name(name: str) -> str | None:
    lower = name.lower()
    # 1차: 상품 자체 키워드 우선(브랜드보다 앞선다).
    for kws, slug in _KW_SLUG:
        if any(kw.lower() in lower for kw in kws):
            return slug
    # 2차: 상품 키워드가 없을 때만 브랜드로 보조 판정.
    for slug, brands in _BRAND_SLUG.items():
        if any(b.lower() in lower for b in brands):
            return slug
    return None
