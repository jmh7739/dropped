...
MIN_HISTORY_POINTS = 4          # 최소 수집 횟수 (무료 크론 ~3h 간격 → 신규딜 더 빨리 노출)
...
END_DISCOUNT = 0.03
# 최신순 재부상: 진행 중 딜이 지난 수집 대비 이 비율 이상 더 떨어지면
#   '방금 다시 급락'으로 보고 detected_at을 갱신(최신순 상단으로 재노출).
#   너무 낮으면 사소한 변동에 계속 튀어오름 → 3%로 잡음 억제.
REDROP_REFRESH = 0.03
